...
build-all: protoc-bins $(PB_FILES) $(PB_STORM_FILES) $(BIN)
...
gopath/bin/migrate: gopath
	GOPATH=$(GOPATH) go install github.com/golang-migrate/migrate/v4/cmd/migrate@$(MIGRATE_VERSION)

gopath/bin/protoc-gen-go: gopath
	GOPATH=$(GOPATH) go install google.golang.org/protobuf/cmd/protoc-gen-go@$(PROTOC_GEN_GO_VERSION)

gopath/bin/protoc-gen-go-grpc: gopath
	GOPATH=$(GOPATH) go install google.golang.org/grpc/cmd/protoc-gen-go-grpc@$(PROTOC_GEN_GO_GRPC_VERSION)

gopath/bin/protoc-gen-stormrpc: gopath
	GOPATH=$(GOPATH) go install github.com/actatum/stormrpc/cmd/protoc-gen-stormrpc@$(PROTOC_GEN_STORMRPC_VERSION)

gopath/bin/swagger: gopath
