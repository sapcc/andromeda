...
func (s *andromedaF5Store) GetPools() ([]*rpcmodels.Pool, error) {
	// AS3 POST /declare payload must include *all* members (servers)
	res, err := s.rpc.GetPools(context.Background(), &server.SearchRequest{
		Provider:      "f5",
		PageNumber:    0,
		ResultPerPage: 1000, // TODO: make it possible to go over all results
	})
	if err != nil {
		return nil, fmt.Errorf("rpc.GetPools failed: %s", err)
	}
	if res == nil {
